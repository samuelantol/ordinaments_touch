...
const Category = props => {
  let categoryClass = 'mcategory-'+props.name; //navigation.css
  if (props.active) {
    categoryClass = 'mcategory-'+props.name+' mcategory-'+props.name+'-active';
  }
...
const FragmentCircle = props => {
  let circleClass = 'mcircle mcircle-unknown'; //navigation.css

  if (props.empty) {
    circleClass = 'mcircle mcircle-empty';
  } else {
    if (props.discovered) {
      circleClass = 'mcircle mcircle-discovered';
    }
    if (props.active) {
      circleClass = 'mcircle mcircle-active';
    }
    if (props.highlighted) {
      circleClass += ' mcircle-highlighted';
    }
  }
    ...
    return (
      <div>
        <div className="mnavigation">
          <div className='m-map'>DISCOVER MAP</div>
          {this.rowBuilder(this.state.rowCount, this.state.rowLength)}
        </div>
        <div className="mcats">
        <div className='m-highlight'>HIGHLIGHT CATEGORY</div>
          {this.categoriesBuilder()}
        </div>
        <div className="msecondary">
